...
describe Agents::DktNerAgent do
  before(:each) do
    @valid_options = Agents::DktNerAgent.new.default_options.merge('models' => 'testmodel')
  ...
  describe "#receive" do
    ...
    it "creates an event after a successfull request" do
      stub_request(:post, "http://dev.digitale-kuratierung.de/api/e-nlp/namedEntityRecognition?analysis=temp&language=en&models=testmodel&outformat=turtle").
