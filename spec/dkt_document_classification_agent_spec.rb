...
describe Agents::DktDocumentClassificationAgent do
  before(:each) do
    @valid_options = Agents::DktDocumentClassificationAgent.new.default_options
    @checker = Agents::DktDocumentClassificationAgent.new(:name => "somename", :options => @valid_options)
  ...
  describe "#receive" do
    ...
    it "creates an event after a successfull request" do
      stub_request(:post, "http://dev.digitale-kuratierung.de/api/e-documentclassification?informat=text/plain&input=Hello%20from%20Huginn&language=en&modelName=3pc&outformat=turtle").
